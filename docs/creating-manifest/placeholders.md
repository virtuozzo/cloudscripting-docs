...
- `${nodes.}`
    ...
    - `(key)` - name of the applied parameter, according to the following list:
        ...
        - `scalingMode` - stateless or stateful [scaling](https://docs.jelastic.com/horizontal-scaling) mode, the possible values are *'NEW'* or *'CLONE'* respectively. The default value is *'CLONE'* for *nodeGroup* [bl,cp,vds] and for the rest of *nodeGroup* types the default value is *'NEW'*.
